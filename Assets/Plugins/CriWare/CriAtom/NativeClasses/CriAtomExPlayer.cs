...
public class CriAtomExPlayer : CriDisposable
{
	 ...
	 * <param name="size">バッファサイズ</param>
	 * \par 説明:
	 * 詳細は ::CriAtomExPlayer::SetData(byte[], int) を参照してください。
	 * \sa CriAtomExPlayer::SetData(byte[], int)
	 */
	 ...
	 * \par 説明:
	 * ::CriAtomExPlayer::EntryData 等を利用した連結再生を行うためのエントリプールを作成します。<br/>
	 * \attention
	 * 本関数を呼び出すと、プレーヤは連結再生用の再生モードになります。<br/>
	 ...
	 * \sa CriAtomExPlayer::PrepareEntryPool, CriAtomExPlayer::EntryData
	 */
	 ...
	 * \par 説明:
	 * 連結再生用に音声ファイルを入力します。<br/>
	 * 連結再生を行う場合には、先頭データを ::CriAtomExPlayer::SetData 関数等で
	 * 設定して再生開始後、本関数により追加の再生用データを入力してください。
	 * \sa CriAtomExPlayer::PrepareEntryPool, CriAtomExPlayer::EntryContentId, CriAtomExPlayer::EntryData
	 */
	 ...
	 * <returns>データ入力に成功したか</returns>
	 * \par 説明:
	 * 連結再生用にコンテンツを入力します。<br/>
	 * CRI File Systemライブラリを使用してCPKファイル内のコンテンツファイルを
	 * ID指定で再生するために使用します。<br/>
	 * 連結再生を行う場合には、先頭データを ::CriAtomExPlayer::SetData 関数等で
	 * 設定して再生開始後、本関数により追加の再生用データを入力してください。
	 * \sa CriAtomExPlayer::PrepareEntryPool, CriAtomExPlayer::EntryFile, CriAtomExPlayer::EntryData
	 */
	 ...
	 * 連結再生用データを入力します。<br/>
	 * 連結再生を行う場合には、先頭データを ::CriAtomExPlayer::SetData 関数等で
	 * 設定して再生開始後、本関数によりデータを入力してください。
	 * \sa CriAtomExPlayer::PrepareEntryPool, CriAtomExPlayer::EntryFile, CriAtomExPlayer::EntryContentId
	 */
	 ...
	 * 連結再生用データを入力します。<br/>
	 * 連結再生を行う場合には、先頭データを ::CriAtomExPlayer::SetData 関数等で
	 * 設定して再生開始後、本関数によりデータを入力してください。<br/>
	 * \sa CriAtomExPlayer::PrepareEntryPool
	 */
 ...
 */
	 ...
	 * \par 説明:
	 * プレーヤにAISACをアタッチ（付与）します。
	 * AISACをアタッチすることにより、キューやトラックにAISACを設定していなくても、AISACの効果を得ることができます。<br/>
	 * 本関数でAISACをアタッチ後、::CriAtomExPlayer::Start 関数により再生開始すると、アタッチしたAISACを考慮して、各種パラメータが適用されます。<br/>
	 ...
	 * <br/>
	 * 本パラメータは ::CriAtomExPlayer::ResetParameters 関数にてクリアされます。
	 * \attention
	 ...
	 * \par 説明:
	 * プレーヤからAISACをデタッチ（分離）します。<br/>
	 * 本関数でAISACをデタッチ後、::CriAtomExPlayer::Start 関数により再生開始すると、デタッチしたAISACの影響は受けなくなります。<br/>
	 ...
	 * <br/>
	 * AISACコントロール値の扱いは::CriAtomExPlayer::SetAisacControl 関数と同様です。<br/>
	 * \par 例:
	 * \code
	 * 	：
	 * // プレーヤの作成
	 ...
	 * \endcode
	 * \sa CriAtomExPlayer::Start, CriAtomExPlayer::Update, CriAtomExPlayer::UpdateAll,
	 * CriAtomExPlayer::SetAisacControl(uint, float)
	 */
	 ...
	 * 	.
	 * \sa CriAtomExPlayer::Start, CriAtomExPlayer::Update, CriAtomExPlayer::UpdateAll,
	 * CriAtomExPlayer::SetAisacControl(string, float)
	 */
	 ...
	 * \attention
	 * 無効なインデックスを指定した場合、falseが返ります。<br/>
	 */
	 ...
	 * \par 説明:
	 * セレクタ名とラベル名を指定して、プレーヤに設定します。１つのプレイヤーに対して最大８つまで設定可能です<br>
	 ...
	 * ラベル情報を含む全てのプレーヤ設定値削除は ::CriAtomExPlayer::ResetParameters を実行してください。<br>
	 * \sa CriAtomExPlayer::ClearSelectorLabels, CriAtomExPlayer::ResetParameters
	 */
	 ...
	 * 本関数を実行すると、プレーヤは以降指定されたボイスプール識別子に一致するボイスプールからのみボイスを取得します。
	 * \sa CriAtomExStandardVoicePool, CriAtomExWaveVoicePool
	 */
	 ...
	 * \sa CriAtomExVoicePool.AttachDspTimeStretch, CriAtomExPlayer.SetVoicePoolIdentifier
	 */
	 ...
	 * \par 説明:
	 * DSPピッチシフタのピッチシフト量を設定します。<br>
	 * 単位はセントです。<br>
	 * 設定可能な値の範囲は -2400 ～ 2400 までです。<br>
	 * 原音と比べて 1200 で 2倍、-1200 で 1/2倍 のピッチシフトになります。<br>
	 * \sa CriAtomExVoicePool.AttachDspPitchShifter, CriAtomExPlayer.SetVoicePoolIdentifier
	 */
	 ...
	 * \sa CriAtomExVoicePool.AttachDspTimeStretch, CriAtomExVoicePool.AttachDspPitchShifter, .SetVoicePoolIdentifier
	 */
	 ...
	 * \sa CriAtomExVoicePool.AttachDspTimeStretch, CriAtomExPlayer.SetDspParameter
	 */
