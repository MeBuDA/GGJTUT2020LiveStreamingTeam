 ...
 * \par 説明:
 * CriAtomSource/CriAtomExPlayerごと、またはバスごとの音声出力の解析を行います。<br/>
 * レベルメータ機能などを提供します。<br/>
 * \attention 注意：
 * CriAtomSource/CriAtomExPlayerにアタッチする場合、HCA-MXやプラットフォーム固有の
...
public class CriAtomExOutputAnalyzer : CriDisposable
{
	...
	public struct Config
	{
		 ...
		 * 出力データの取得を有効化します。
		 * 機能を利用する場合、本フラグにtrueを指定すると同時に、
		 * numCapturedPcmSamplesに正数を指定してください。
		 */
		 ...
		 * \par 説明：
		 * 出力データ取得用のコールバックを有効化します。<br/>
		 * 機能を利用する場合、本フラグにtrueを指定した上で、MonoBehaviour.Update 内等で
		 * 定期的に ExecutePcmCaptureCallback を呼び出してください。<br/>
		 */
	 ...
	 * </code>
	 * \par 備考：
	 * 解析モジュールにアタッチ可能なCriAtomSource/CriAtomExPlayer/バスは一つのみです。<br/>
	 ...
	 * <returns>アタッチが成功したかどうか（成功：true、失敗：false）</returns>
	 * \par 説明:
	 * 出力データ解析を行うDSPバスをアタッチします。<br/>
	 * 複数のDSPバスをアタッチすることは出来ません。
	 * アタッチ中に別のDSPバスをアタッチした場合、アタッチ中のDSPバスはデタッチされます。<br/>
	 ...
	 * \par 説明：
	 * スペクトラムアナライザによって解析された帯域ごとの振幅値を取得します。<br/>
	 ...
	 * \par 説明:
	 * アタッチ中の音声出力の波形データを取得します。<br/>
	 * 本機能を利用する場合、ConfigのenablePcmCaptureにtrueを、numCapturedPcmSamplesに
	 * 正数を指定してモジュールを作成してください。<br/>
	 * \attention
	 * 引数の配列の長さが十分でない場合、関数内で配列の確保が行われます。<br/>
	 * 不要なGCの発生を避けるため、初期化コンフィグで指定したデータサンプル数以上の
	 * 長さの配列を引数に渡すようにしてください。
	 * 現状、取得可能なチャンネルはL/Rのみです。chには0か1を指定してください。
	 * \sa CriAtomExOutputAnalyzer::AttachExPlayer, CriAtomExOutputAnalyzer::AttachDspBus
	 */
	 ...
	 * \par 説明：
	 * 出力データ取得用のコールバックを実効します。<br/>
	 * 本関数を呼び出すと、最後の実行時からの出力差分データを引数とするコールバックが
	 * 複数回呼び出されます。<br/>
	 * \attention
	 * コールバックによる波形データ取得を利用する場合、本関数を定期的に呼び出してください。<br/>
	 * 本関数が長時間呼び出されなかった場合、取得可能な波形データに欠落が生じます。<br/>
	 */
